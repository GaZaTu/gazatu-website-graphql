...
type MergedReducer<T> = Reducer<InferStateType<T>, InferActionTypes<InferReducerTypes<T>>>

const mergeReducers = <T extends ReducersMapObject<any, any>>(reducers: T): MergedReducer<T> => {
  return (state, action) => {
