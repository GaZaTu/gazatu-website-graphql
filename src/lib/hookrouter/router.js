 ...
 * @param {{ [key: string]: any; }} [queryParams] Key/Value pairs to convert into get parameters to be appended to the URL.
 * @param {boolean} [replaceQueryParams=true] Should existing query parameters be carried over, or dropped (replaced)?
 * @param {boolean} [force=false] navigate even when the url is the same
 */
export const navigate = (url, replace = false, queryParams = null, replaceQueryParams = true, force = false) => {
	url = interceptRoute(currentPath, resolvePath(url));

  if (!url || (url === currentPath && !force)) {
		return;
	}
...
export const getWorkingPath = (parentRouterId) => {
	if (!parentRouterId) {
		return window.location.pathname.replace(basePathRegEx, '') || '/';
	}
 ...
 * to that.
 * @param {{ [key: string]: (params: QueryParams) => any; }} routeObj {"/someRoute": () => <Example />}
 */
